...
object BeLocale : Locale() {
    override val start =
        regular("Прывітанне! Мяне клічуць ") + bold("PrizeBot") +
            regular(
                ", тут можна празрыста разыграць прызы. " +
            ...
            link(text = "тут", url = "https://github.com/y9san9/prizebot") +
            " можна паглядзець зыходны код.\n" +
    ...
    override val help =
        "Прывітанне! Я прасунуты бот для розыгрышаў, вось спіс даступных загадаў:\n" +
    ...
    override val highLoadMessage =
        "Зараз бот знаходзіцца пад вялікай нагрузкай, трывайся. " +
    ...
    override fun winner(plural: Boolean) =
        if (plural) "Пераможцы" else "Пераможца"
    ...
    override fun confirmation(confirmationText: String) =
        "Ты ўпэўнены, што хочаш $confirmationText?"
    ...
    override val enterRaffleDateInput =
        regular(
            "Увядзі дату для аўтаматычнага розыгрышу ў адным з фарматаў: ",
        ) +
            bold("00:00") + ", " +
            bold("00:00 13.01") + ", " +
            bold("00:00 13.01.2020") +
            " (націсні /skip каб прапусціць, або /cancel каб скасаваць). " +
    ...
    override val customTimezoneInput =
        regular("Увядзі часавы пояс у адным з фарматаў: ") +
            bold("+9") + ", " + bold("-9:30")
    ...
    override val channelLinkingHelp =
        bold(
            "Каб далучыць канал або чат вам патрэбна выканаць наступныя крокі:\n\n",
        ) +
            "• Дадаць @y9prizebot у якасці адміністратара да канала/чата, які патрэбна далучыць (" +
            bold("у яго павінна быць імя") +
            ", каб кожны мог далучыцца да яго), " +
    ...
    override fun subscribeTo(username: String) =
        regular("Падпісацца на ") + bold(username)

    override fun inviteFriends(count: Int) =
        regular("Запрасіць ") + bold("$count") +
            " ${getValidFriendsForm(count)} да ўдзелу"
    ...
    override fun friendsAreNotInvited(invitedCount: Int, requiredCount: Int) =
        "Вы запрасілі $invitedCount / $requiredCount сяброў"
    ...
    override val displayWinnersWithEmoji =
        "Паказваць пераможцаў з эмодзі? Напрыклад:\n\n" +
