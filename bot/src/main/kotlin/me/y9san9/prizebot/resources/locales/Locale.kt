...
open class Locale {
    ...
    open val help =
        "Hey! I am an advanced bot for giveaways, here is the available commands list:\n" +
    ...
    open val highLoadMessage =
        "The bot right now is receiving lots of requests, so please be patient. " +
    ...
    open fun confirmation(confirmationText: String) =
    ...
    open val enterRaffleDateInput =
        regular("Enter auto-raffle date in one of these formats: ") +
            bold("00:00") + ", " +
            bold("00:00 13.01") + ", " +
    ...
    open val customTimezoneInput =
        regular("Enter a timezone in one of this formats: ") +
            bold("+9") + ", " + bold("-9:30")
    ...
    open val channelLinkingHelp =
        bold("To link a channel you should follow these steps:\n\n") +
            "• Add @y9prizebot as administrator to the channel/chat you want to link (" +
            bold("it must have a username") +
    ...
    open fun subscribeTo(username: String) =
        regular("Subscribe to ") + bold(username)

    open fun inviteFriends(count: Int) =
        regular("Invite ") + bold("$count") + " friend" +
            (if (count > 1) "s" else "") + " in giveaway"

    open val channelConditionRequiredForInvitations =
        "At least one channel subscription is required if you want to add " +
            "Friend invitations"
    ...
    open fun friendsAreNotInvited(invitedCount: Int, requiredCount: Int) =
        "You have invited $invitedCount / $requiredCount friends"
    ...
    open val displayWinnersWithEmoji =
        "Display the winners with emojis like this?\n\n" +
