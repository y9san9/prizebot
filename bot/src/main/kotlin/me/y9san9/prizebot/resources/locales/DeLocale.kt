...
object DeLocale : Locale() {
    ...
    override val help =
        "Hey, ich bin ein fortgeschrittener Bot für Verlosungen, hier ist die Liste der verfügbaren Befehle: \n" +
    ...
    override val highLoadMessage =
        "Der Bot ist derzeit stark ausgelastet, also habe bitte etwas Geduld. " +
    ...
    override fun winner(plural: Boolean) =
        if (plural) "Gewinner" else "Gewinner"
    ...
    override fun confirmation(confirmationText: String) =
        "Bist du sicher, du möchtest $confirmationText"
    ...
    override val enterRaffleDateInput =
        regular(
            "Gib das Datum für die automatische Verlosung in einem dieser Formate ein: ",
        ) +
            bold("00:00") + ", " +
            bold("00:00 13.01") + ", " +
            bold("00:00 13.01.2020") +
            " (Nutze /skip um zu überspringen oder /cancel um abzubrechen). " +
    ...
    override val `UTC-3` = "Brasilien -3"
    ...
    override val customTimezoneInput =
        regular("Gib die Zeitzone in einem dieser Formate ein: ") +
            bold("+9") + ", " + bold("-9:30")
    ...
    override val channelLinkingHelp =
        bold(
            "Um einen Kanal zu verlinken solltest du diese Schritte befolgen:\n\n",
        ) +
            "• Füge @y9prizebot als Administrator dem Kanal/Chat deiner Wahl hinzu (" +
            bold("Er muss einen Benutzernamen haben") +
            " damit sich jeder anschliessen kann), " +
    ...
    override fun friendsAreNotInvited(invitedCount: Int, requiredCount: Int) =
        "Du hast $invitedCount / $requiredCount Freunde eingeladen"
    ...
    override val displayWinnersWithEmoji =
        "Sollen die Gewinner mit Emojis wie diesen dargestellt werden?\n\n" +
