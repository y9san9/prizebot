...
object RuLocale : Locale() {
    ...
    override val channelLinkingHelp = underline("Чтобы привязать канал вам нужно выполнить следующие 4 шага:\n\n") +
            "• Добавить @secure_prize_bot в ваш канал с " + bold("username") + ", который вы хотите привязать (чтобы любой мог присоединиться к нему), позже это будет использовано для проверки участников\n" +
